import { BASELINE_COLUMN_NAMES, sanitizeIdentifier } from "../index.js";
export class AttributeCatalog {
    ...
    `);
        for (const row of rows) {
            ...
            this.entries.set(key, {
                key,
                sanitizedKey: expectString(row["sanitized_key"], "attribute_catalog.sanitized_key"),
                ...
                nonNullRows: expectNumber(row["non_null_rows"], "attribute_catalog.non_null_rows"),
                firstSeenAt: expectString(row["first_seen_at"], "attribute_catalog.first_seen_at"),
    ...
    getPromotedColumns() {
        ...
        for (const entry of this.entries.values()) {
            if (entry.storageState === "promoted" &&
    ...
    listColumns(schema) {
        const baselineColumns = schema
            .listActualColumns()
            .filter((column) => BASELINE_COLUMN_NAMES.includes(column.name) &&
        ...
        const dynamicColumns = [...this.entries.values()]
            .map((entry) => ({
            name: entry.key,
            storageState: entry.storageState === "promoting" ? "overflow_only" : entry.storageState,
    ...
    async recordRows(database, rows) {
    ...
    selectPromotionCandidates(totalRetainedRows, minRows, minRatio, limit) {
        if (totalRetainedRows <= 0) {
            return [];
        }
        return [...this.entries.values()]
            .filter((entry) => entry.storageState === "overflow_only" &&
            ...
            .sort((left, right) => right.nonNullRows - left.nonNullRows ||
            right.lastSeenAt.localeCompare(left.lastSeenAt))
            .slice(0, limit);
    }
    async markPromoting(database, key) {
    ...
    async markPromoted(database, key, promotedColumn, promotedType) {
        const now = new Date().toISOString();
        const entry = this.entries.get(key) ?? {
            key,
    ...
    async markFailed(database, key, error) {
        const entry = this.entries.get(key);
        if (!entry) {
            return;
        }
...
function applyRowAttributes(updates, attributes, now, existingEntries) {
...
function nonNullValueType(value) {
...
export function mergeInferredType(current, next) {
    if (current === next) {
        return current;
    }
    ...
    if ((current === "BIGINT" && next === "DOUBLE") ||
        (current === "DOUBLE" && next === "BIGINT")) {
        return "DOUBLE";
    }
    return "JSON";
}
async function persistEntry(database, entry) {
    await database.execute(`INSERT INTO attribute_catalog (
      key,
      sanitized_key,
      storage_state,
      inferred_type,
...
function expectStorageState(value) {
    if (value === "overflow_only" ||
        value === "promoting" ||
        value === "promoted" ||
        value === "failed") {
        return value;
...
function expectInferredType(value) {
    if (value === "BOOLEAN" ||
        value === "BIGINT" ||
        value === "DOUBLE" ||
        value === "VARCHAR" ||
...
function expectNullableInferredType(value) {
...
//# sourceMappingURL=attribute-catalog.js.map